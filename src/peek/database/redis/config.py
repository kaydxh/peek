...
class RedisConfig(BaseModel):
    ...
    addresses: List[str] = Field(
        default_factory=lambda: ["localhost:6379"], description="Redis 地址列表"
    )
    username: str = Field(default="", description="用户名")
    password: str = Field(default="", description="密码")
    db: int = Field(default=0, ge=0, description="数据库编号")
    max_connections: int = Field(
        default=100, ge=0, description="最大连接数（对应 Go 版本 pool_size）"
    )
    max_idle_connections: int = Field(
        default=10, ge=0, description="最小空闲连接数（对应 Go 版本 min_idle_conns）"
    )
    ...
    max_wait_duration: float = Field(
        default=20.0, ge=0, description="等待连接最大时间（秒）"
    )
    fail_after_duration: float = Field(
        default=300.0, ge=0, description="超过此时间后放弃连接（秒）"
    )
    master_name: str = Field(
        default="mymaster", description="Sentinel 模式的 master 名称"
    )
    ssl: bool = Field(default=False, description="是否启用 SSL")
    health_check_interval: int = Field(
        default=30, ge=0, description="连接池健康检查间隔（秒），0 表示禁用"
    )
